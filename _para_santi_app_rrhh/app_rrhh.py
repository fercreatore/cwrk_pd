...
def _get_activos(df_emp, df_sueldos=None, meses=3):
    """Activos = cobraron sueldo en los últimos `meses` meses."""
    if df_emp.empty:
        return pd.DataFrame()
    if df_sueldos is None or df_sueldos.empty:
        # fallback al campo ERP si no hay datos de sueldos
        return df_emp[(df_emp["motivo_baja"] != "B") | (df_emp["motivo_baja"].isna())].copy()
    corte = pd.Timestamp.now() - pd.DateOffset(months=meses)
    recientes = df_sueldos[pd.to_datetime(df_sueldos["fecha_contable"]) >= corte]
    numeros_activos = recientes["numero_cuenta"].unique()
    return df_emp[df_emp["numero"].isin(numeros_activos)].copy()
...
def render_nomina():
    ...
    activos = _get_activos(df_emp, df_sueldos)
    if activos.empty:
    ...
    with tab_sueldos:
        if df_sueldos.empty:
            ...
            emp_map = {int(r["numero"]): str(r["denominacion"]) for _, r in activos.iterrows()}
            emp_sel = st.selectbox(
                "Empleado",
                list(emp_map.keys()),
                format_func=lambda x: emp_map.get(x, str(x))
            )
            ...
            if hist.empty:
                ...
                NOMBRE_MOV = {
                    10: "Sueldo bruto", 7: "Pago neto", 11: "Aportes",
                    6: "Adelanto", 12: "Otros", 8: "Vacaciones", 9: "SAC",
                }
                hist["fecha"] = pd.to_datetime(hist["fecha_contable"]).dt.strftime("%Y-%m-%d")
                hist["concepto"] = hist["codigo_movimiento"].apply(
                    lambda c: NOMBRE_MOV.get(int(c), f"Cod {int(c)}")
                )
                hist_display = hist[["fecha", "concepto", "importe", "operacion"]].copy()
                hist_display.columns = ["Fecha", "Concepto", "Importe", "Op"]
                st.dataframe(
...
def render_overview():
    ...
    activos = _get_activos(df_emp, df_sueldos)
    bajas = df_emp[~df_emp["numero"].isin(activos["numero"])].copy()
