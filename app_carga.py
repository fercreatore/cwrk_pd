...
def _detectar_proveedor_por_articulos(codigos: list) -> dict | None:
    """
    Dado una lista de códigos de artículo (modelo/referencia del Excel),
    busca en la BD cuál es el proveedor que tiene esos artículos.
    Retorna dict con config del proveedor o None.
    """
    if not codigos:
        return None
    try:
        # Tomar hasta 10 códigos para buscar
        muestra = codigos[:10]
        ...
        sql = f"""
            ...
            ORDER BY hits DESC
        """
        params = muestra + muestra  # una vez para descripcion_1, otra para codigo
        ...
        with _get_conn() as conn:
            ...
            if row and row.proveedor:
                ...
                if prov_base:
                    ...
                    prov.update(pricing)
                    prov["_fuente"] = "artículos BD"
                    return prov
    except Exception as e:
        import logging
        logging.getLogger("app_carga").warning(f"Error detectando proveedor por artículos: {e}")
    return None
...
""")
    ...
    if prov_id:
        ...
        prov_config = {}
        marca_default = 0
        st.sidebar.info("Cargá un archivo para detectar el proveedor automáticamente, o seleccioná uno de la lista.")

    # Mostrar info del proveedor (solo si hay uno seleccionado)
    if prov_id:
        st.sidebar.markdown(f"""
**Nro:** {prov_id}
...
# Default: Nota de Pedido (index=2) — es lo más seguro y común
factura_ocr = st.session_state.get("factura_ocr")
tipo_default = 2  # NP por defecto
nro_default = ""
...
# Empresa destino
empresa_sel = st.sidebar.radio("Empresa destino", ["H4", "CALZALINDO"], index=0)
...
def _procesar(prov_id, prov_nombre, marca_id, nro_factura, fecha, tipo, modo_test):
    """Procesa la factura: busca/crea artículos y luego inserta el comprobante."""
    if not CARGA_DISPONIBLE:
        st.error("Módulo paso8_carga_factura no disponible")
        return
    ...
    if modo_test:
        ...
        for linea in factura.lineas:
            ...
            try:
                existente = buscar_articulo_por_sinonimo(
                    sinonimo, proveedor=prov_id,
                    modelo=linea.modelo, color=linea.color,
                    talle=linea.talle)
                if existente:
        ...
        if resultado["fallidos"] > 0:
            st.warning(f"⚠️ {resultado['fallidos']} artículos fallaron — revisa errores antes de continuar")
            st.session_state.resultado = resultado
            st.rerun()
            return

        # Crear el comprobante (NP o RM)
        if tipo in ("NP", "RM"):
            with st.spinner(f"Insertando {'remito' if tipo == 'RM' else 'nota de pedido'}..."):
                res_comp = crear_pedido_desde_factura(
                    factura, resultado, tipo=tipo,
                    empresa=empresa_sel,
                    sucursal_remito=sucursal_remito,
                    numero_remito=numero_remito,
                    deposito=deposito_remito,
                    dry_run=False,
                )
            if res_comp.get("exitoso"):
            ...
            st.success(f"✅ Artículos procesados — JSON: {path}")
    ...
    if st.session_state.articulos_lista:
        ...
        with col_btn1:
            if CARGA_DISPONIBLE and st.button("🔍 Verificar Artículos", use_container_width=True, type="secondary"):
                if not prov_id:
                    st.error("Seleccioná un proveedor antes de verificar.")
                else:
        ...
        with col_btn2:
            ...
            if CARGA_DISPONIBLE and st.button(btn_label, use_container_width=True, type=btn_type):
                if not prov_id:
                    st.error("Seleccioná un proveedor antes de procesar.")
                else:
