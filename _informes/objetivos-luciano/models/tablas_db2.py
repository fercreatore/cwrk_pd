...
db2.define_table('sucursal',
	Field('nombre','string'),
	Field('activa','boolean',default=False))

# SECTORES DE ATENCION
db2.define_table('sectores',
	Field('descripcion','string',20, requires=IS_NOT_EMPTY()),
	Field('activo','boolean', default=True))

# LETRAS PARA LOS TICKETS
db2.define_table('letras',
	Field('letra','string',1),
	Field('sector_default','reference sectores', requires=IS_EMPTY_OR(IS_IN_DB(db2, db2.sectores.id,'%(descripcion)s'))),
	Field('texto','string'),
	Field('mostrar','boolean', default=True))


db2.define_table('turnos',
