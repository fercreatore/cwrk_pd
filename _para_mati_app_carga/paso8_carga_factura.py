...
def construir_sinonimo(modelo: str, color_code: str, talle: str,
    ...
    Ejemplo: proveedor=656, modelo="RBK1100033358", color=13(GRIS), talle=40
             → strip RBK → "1100033358" → últimos 5 dígitos → "33358"
             → "656333581340"
    ...
    Prioridad para cod5:
      ...
      3. Fallback para códigos largos (Reebok/etc): últimos 5 dígitos del código
      4. Fallback genérico: limpia el modelo (strippea prefijos de marca)
    ...
    if not cod_prov:
        ...
        # (código largo del proveedor), no sirve para 5 chars. Se usan los
        # últimos 5 dígitos del código numérico como cod_prov.
        is_long_ref = False
        ...
        if is_long_ref:
            # Para Reebok (656) y similares: el modelo viene como "RBK1100033358"
            # (código largo del proveedor). Usar los últimos 5 dígitos del código
            # numérico como cod_prov. Esto es determinístico y consistente.
            # Ej: "RBK1100033358" → strip "RBK" → "1100033358" → últimos 5 → "33358"
            stripped = mod_clean
            for prefix in ["RBK-", "RBK", "WKC", "WK-", "WK"]:
            ...
            # Extraer solo dígitos para tomar los últimos 5
            digits_only = "".join(c for c in stripped if c.isdigit())
            if len(digits_only) >= 5:
                cod_prov = digits_only[-5:]
                log.info(f"  cod5 desde últimos 5 dígitos del código largo: '{cod_prov}' ← '{mod_clean}'")
            elif digits_only:
                # Menos de 5 dígitos: right-pad con ceros
                cod_prov = digits_only
                log.info(f"  cod5 desde dígitos del código largo: '{cod_prov}' ← '{mod_clean}'")
            else:
                # Sin dígitos: fallback a últimos 5 alfanuméricos
                cod_prov = stripped[-5:] if len(stripped) >= 5 else stripped
                log.info(f"  cod5 fallback alfanumérico: '{cod_prov}' ← '{mod_clean}'")
